...
Canonical Founder feed, source overview, and aggregate dashboard surfaces.
"""
from typing import Sequence, Union

from alembic import op

revision: str = "0015_hosted_founder_surface"
...
def upgrade() -> None:
    # PostgreSQL cannot add columns to a view with CREATE OR REPLACE VIEW, so
    # replace the old one-row-per-projection surface atomically in the
    # migration transaction. The underlying tables retain their existing RLS.
    op.execute("DROP VIEW IF EXISTS public.founder_feed")
    op.execute(_FOUNDER_FEED_SQL)
    ...
    for view in ("founder_feed", "founder_source_overview"):
        ...
        op.execute(f"""DO $$ BEGIN
          ...
          END IF;
        END $$""")
    op.execute("REVOKE ALL ON FUNCTION public.founder_dashboard_daily(integer, double precision) FROM PUBLIC")
    op.execute("""DO $$ BEGIN
      IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname='anon') THEN
...
def downgrade() -> None:
    op.execute("DROP FUNCTION IF EXISTS public.founder_dashboard_daily(integer, double precision)")
    op.execute("DROP VIEW IF EXISTS public.founder_source_overview")
    op.execute("DROP VIEW IF EXISTS public.founder_feed")
    op.execute("""
    ...
    """)
    op.execute("REVOKE ALL ON public.founder_feed FROM PUBLIC")
    op.execute("""DO $$ BEGIN
      IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname='authenticated') THEN
