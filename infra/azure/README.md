...
5. Run `DEPLOY_STAGING` with `acknowledge_staging_deployment=true`. The
   workflow deploys the migration job, starts it once, waits for that execution to reach `Succeeded`, then deploys API, worker, and scheduler.
...
`AZURE_SUBSCRIPTION_ID`) and application secret names only. The Truth Pack URI is treated as a secret capability and is injected through a Container Apps secret reference, not a plain template/environment value. Secret values are
